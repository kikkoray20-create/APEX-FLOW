        ...
        <div className="flex flex-col space-y-8 animate-in fade-in duration-500 pb-10">
            {isCreating ? (
                <div className="flex flex-col space-y-8 animate-in slide-in-from-right duration-500">
                    {/* CREATE GR PAGE VIEW */}
                    <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[85vh] no-print">
                        <header className="bg-white border-b border-slate-100 shrink-0">
                            <div className="px-6 md:px-10 h-24 flex items-center justify-between">
                                <div className="flex items-center gap-6">
                                    <button 
                                        onClick={() => setIsCreating(false)}
                                        className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-400 hover:text-rose-600 hover:border-rose-200 transition-all active:scale-95"
                                    >
                                ...
                                <div className="hidden md:flex items-center gap-3">
                                    {[1, 2, 3].map((s) => (
                                        <div 
                                            key={s}
                                            className={`w-3 h-3 rounded-full transition-all duration-300 ${step >= s ? 'bg-rose-600 w-8' : 'bg-slate-200'}`}
                                        />
                                    ))}
                                </div>
                            </div>
                        ...
                        {step === 1 ? (
                            <div className="flex-1 p-6 md:p-12 flex flex-col min-h-0 bg-slate-50/30">
                                <div className="relative mb-8 max-w-2xl mx-auto w-full group">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-14 pr-8 py-4 bg-white border border-slate-200 rounded-[2rem] outline-none font-bold text-sm focus:ring-8 focus:ring-rose-50 transition-all shadow-sm" 
                                    />
                                </div>
                                <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar max-w-4xl mx-auto w-full">
                                    {customers.filter(c => c.name.toLowerCase().includes(customerSearch.toLowerCase()) || c.phone.includes(customerSearch)).map(customer => (
                                        <button key={customer.id} onClick={() => { setSelectedCustomer(customer); setStep(2); }} className="w-full bg-white p-4 rounded-3xl border border-slate-100 flex items-center justify-between hover:border-rose-400 hover:translate-x-1 transition-all group shadow-sm">
                                            <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest group-hover:text-rose-500">Select</span>
                                                <ChevronRight size={18} className="text-slate-200 group-hover:text-rose-500" />
                                            </div>
                                        </button>
                                    ))}
                                </div>
                                ...
                                <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-2 custom-scrollbar pb-32">
                                    {!isDirectMode ? (
                                        <div className="flex flex-col gap-2 max-w-6xl mx-auto w-full">
                                            {/* List Header */}
                                            <div className="px-6 py-2 flex items-center text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-50 mb-2">
                                                <div className="w-[15%]">Brand / Quality</div>
                                                <div className="flex-1">Model Identity</div>
                                                <div className="w-32 text-right pr-8">Return Rate (₹)</div>
                                        ...
                                        <div className="max-w-xl mx-auto w-full p-8 bg-white rounded-[2.5rem] shadow-lg border border-slate-100 space-y-6 mt-6">
                            ...
                            <div className="flex-1 p-6 md:p-16 bg-[#f8fafc] overflow-y-auto custom-scrollbar animate-in fade-in duration-300">
                                <div className="max-w-4xl mx-auto space-y-10">
                                    <button onClick={() => setStep(2)} className="flex items-center gap-3 text-slate-400 hover:text-rose-600 font-black text-[11px] uppercase tracking-widest transition-colors">
                                        <ArrowLeft size={18} strokeWidth={3} /> Adjust Selection Payload
                                    </button>
                                    <div className="bg-white rounded-[3.5rem] p-12 shadow-2xl border border-slate-100">
                                        <div className="flex items-center gap-6 mb-12">
                                            ...
                                            <div>
                                                <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter leading-none">GR Submission Summary</h3>
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Node Verification Step</p>
                                            </div>
                                        </div>
                                        <div className="space-y-6 mb-16">
                                            {!isDirectMode ? (
                                                        ...
                                                        return (
                                                            <div key={id} className="flex justify-between items-center py-5">
                                                                <div className="min-w-0 pr-6">
                                                                    <p className="text-[15px] font-black text-slate-800 uppercase truncate leading-none">{i.brand} {i.model}</p>
                                                ...
                                                <div className="py-16 text-center space-y-6 bg-slate-50/50 rounded-[3rem] border border-slate-100">
                                                    <div className="inline-flex px-8 py-3 bg-rose-100 text-rose-600 border border-rose-200 rounded-full text-[11px] font-black uppercase tracking-widest">DIRECT CREDIT VOUCHER</div>
                                                    <p className="text-3xl font-black text-slate-700 uppercase tracking-tight px-10">"{directRemarks || 'Voucher Adjustment Entry'}"</p>
                                                </div>
                                            )}
                                            <div className="pt-10 flex justify-between items-center border-t border-slate-200 border-dashed">
                                                <div>
                                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Final Balance Credit Allocation</span>
                                                    <p className="text-[9px] font-bold text-slate-300 uppercase mt-1">Authorized Digital Transaction</p>
                                                </div>
                                                <span className="text-6xl font-black text-rose-600 italic tracking-tighter">₹{totalCreditValue.toFixed(1)}</span>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                                            <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-inner flex flex-col justify-center">
                                                <div className="flex items-center gap-3 mb-3">
                                                    ...
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Recipient Customer Node</p>
                                                </div>
                                                <p className="text-xl font-black text-slate-900 uppercase leading-none">{selectedCustomer?.name}</p>
                                                <p className="text-[11px] font-bold text-slate-500 uppercase mt-2">{selectedCustomer?.city} | {selectedCustomer?.phone}</p>
                                            </div>
                                            <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-inner flex flex-col justify-center items-center text-center">
                                    ...
                                    <div className="text-center">
                                        <p className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.5em]">Certified Digital Transmission Endpoint</p>
                                    </div>
                                </div>
                            </div>
                        )}
                                            ...
                                            <td className="px-10 py-6 text-right">
                                                <div className="flex items-center justify-end gap-3">
                                                    <button 
                                                        ...
                                                        className="flex flex-col items-center justify-center w-14 h-14 bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-2xl transition-all group shadow-sm active:scale-95"
                                                    >
                                                        <Eye size={18} strokeWidth={3} className="mb-1" />
                                                        <span className="text-[8px] font-black uppercase tracking-widest">VIEW</span>
                                                    </button>
                                                    <button 
                                                        onClick={() => { setGrToDelete(item.id); setIsDeleteConfirmOpen(true); }} 
                                                        className="flex flex-col items-center justify-center w-14 h-14 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-2xl transition-all group shadow-sm active:scale-95"
                                                    >
                                                        <Trash2 size={18} strokeWidth={3} className="mb-1" />
                    ...
                    <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-100 print:shadow-none print:border-none print:rounded-none animate-in zoom-in-95">
                        ...
                        <div className="px-8 py-5 bg-white border-t border-slate-100 flex flex-wrap justify-center sm:justify-end gap-3 no-print shrink-0">
                            ...
                            <button onClick={() => window.print()} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-100 active:scale-95 transition-all hover:bg-indigo-700">
                                <Printer size={16}/> PDF View / Print
                            </button>
